...
use crate::domain::{Command, ParseResult, SPEC_VERSION, TextBlock, Warning};
...
fn step_in_fence(ctx: &mut ParseCtx, joiner: &mut LineJoiner, cmd: PendingCommand) -> LoopAction {
...
fn start_new_command(
    ctx: &mut ParseCtx,
    header: CommandHeader,
    first_physical: usize,
    last_physical: usize,
) {
    let mut cmd = start_command(header, first_physical, ctx.cmd_seq);
    cmd.end_line = last_physical;
    ctx.cmd_seq += 1;
    if cmd.is_open {
        ctx.state = ParserState::InFence(cmd);
    } else {
...
mod tests {
    ...
    fn fence_followed_by_new_command() {
        ...
        assert_eq!(
            result.commands.get(1).unwrap().arguments.mode,
            ArgumentMode::SingleLine
        );
    }
